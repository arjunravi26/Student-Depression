...
def main():
    ...
    Study_Satisfaction = st.number_input(
        "Are you satisfied in your studies,if yes-rate(1-5)")
    job = ['Yes', 'No']
    ...
    Financial_Stress = st.number_input(
        "Enter Financial stress range between 1 and 5")
    Mental = ['No', 'Yes']
    Family_History_of_Mental_Illness = st.selectbox(
        "Is there any family history of mental illness?", Mental)
    ...
    if pred:
        try:
            print(Gender)
            Gender = pickle.load(
                open('artifacts/Gender.sav', 'rb')).transform([Gender])[0]
            Habits = pickle.load(
                open('artifacts/Dietary Habits.sav', 'rb')).transform([Habits])[0]
            City = pickle.load(
                open('artifacts/City.sav', 'rb')).transform([City])[0]
            Profession = pickle.load(
                open('artifacts/Profession.sav', 'rb')).transform([Profession])[0]
            suicide = pickle.load(open(
                'artifacts/Have you ever had suicidal thoughts .sav', 'rb')).transform([suicide])[0]
            course = pickle.load(
                open('artifacts/Degree.sav', 'rb')).transform([course])[0]
            Family_History_of_Mental_Illness = pickle.load(open(
                'artifacts/Family History of Mental Illness.sav', 'rb')).transform([Family_History_of_Mental_Illness])[0]
